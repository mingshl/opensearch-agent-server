...
# Tool Call TypedDicts for better type safety


class ToolCallInfo(TypedDict):
    """Information about a tool call that has been seen.

    This TypedDict tracks tool calls that have been encountered during
    event processing, storing both the serialized and parsed representations.

    Attributes:
        name: Name of the tool that was called
        args: Serialized JSON string of tool arguments
        input: Tool input dictionary (parsed arguments)

    """

    name: str
    args: str  # Serialized JSON string of tool arguments
    input: dict[str, Any]  # Tool input dictionary


class ToolCallInfoInternal(ToolCallInfo, total=False):
    """Internal tool call info with optional extra fields.

    We keep the external `ToolCallInfo` minimal/strict, but internally we sometimes
    need to retain extra metadata for protocol adaptation (e.g., mapping between
    Strands tool IDs and generated frontend tool IDs).

    Attributes:
        strands_tool_id: Optional Strands tool call ID for ID mapping.
    """

    strands_tool_id: str


class ActiveToolCallInfo(TypedDict):
    """Information about an active (in-progress) tool call.

    This TypedDict tracks tool calls that are currently active (started but
    not yet completed), used for managing tool call lifecycle during event processing.

    Attributes:
        name: Name of the tool being called
        input: Tool input dictionary with arguments
...
# Handler Result Types for clearer return semantics


@dataclass(frozen=True)
class EventResult:
    """Result containing an AG-UI event to yield.

    This is the most common result type - handlers yield events that should
    be passed through to the event stream.

    Attributes:
        event: AG-UI event object to yield (AGUIEvent type alias)
        should_stop: Optional flag indicating if streaming should stop after this event
    """
...
@dataclass(frozen=True)
class ToolCallStateUpdate:
    """Result containing tool call state updates.

    Used when tool call handlers need to update both tool_calls_seen and
    active_tool_calls dictionaries.

    Attributes:
        tool_calls_seen: Updated dictionary of seen tool calls
        active_tool_calls: Updated dictionary of active tool calls
        should_stop: Optional flag indicating if streaming should stop (Phase 1: Frontend Tool Detection)
    """
...
@dataclass(frozen=True)
class ToolResultStateUpdate:
    """Result containing tool result state update with optional stop flag.

    Used when tool result handlers need to update active_tool_calls and
    optionally signal that streaming should stop.

    Attributes:
        active_tool_calls: Updated dictionary of active tool calls
        should_stop: Flag indicating if streaming should stop
    """
...
class CompletionSignal:
    """Result signaling that processing is complete.

    Used by ResultEventHandler to signal that active tool calls should
    be completed. The caller handles the actual completion logic.

    Attributes:
        None - this is just a signal
    """

    pass


# Type alias for all handler results (PEP 604 union)
HandlerResult = (
    EventResult | ToolCallStateUpdate | ToolResultStateUpdate | CompletionSignal
)


class StrandsEvent(TypedDict, total=False):
    """TypedDict for Strands agent event dictionaries.

    Strands events can contain various optional fields depending on the event type.
    This TypedDict captures all possible fields that may be present.
    """

    data: str  # Text streaming data
    message: dict[str, Any]  # Message object (user/assistant)
    current_tool_use: dict[str, Any]  # Tool call information
    multiagent_node_start: dict[str, Any]  # Multi-agent node start info
    multiagent_node_stop: dict[str, Any]  # Multi-agent node stop info
    multiagent_handoff: dict[str, Any]  # Multi-agent handoff info
    result: dict[str, Any]  # Final result
    init_event_loop: bool  # Lifecycle event
    start_event_loop: bool  # Lifecycle event
    complete: bool  # Completion flag
    force_stop: bool  # Force stop flag
