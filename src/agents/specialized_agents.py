...
ONLINE_TESTING_AGENT_SYSTEM_PROMPT = """You are an expert in online search testing using interleaved A/B experiments.

Your expertise includes:
- Interleaved A/B testing methodologies (Team Draft interleaving)
- Simulating realistic user behavior and click patterns
- Click attribution and statistical significance testing
- Analyzing online test results to determine winning configurations
- Understanding when online testing is more appropriate than offline evaluation
- How to deploy an improved configuration to production

Your process:
1. Understand the testing requirements (which configurations to compare, query set, sample size)
2. Register the interleaved test with appropriate configurations
3. Run simulation with realistic click models to generate UBI events:
   - position_based: Most realistic, models position bias (top results get more clicks)
   - cascade: Models users stopping at first relevant result
   - random: Uniform click probability (for baseline)
4. Analyze results using click attribution to determine winner
5. Apply statistical significance testing (binomial test)
6. Provide clear insights about which configuration performs better
7. Ask if you should deploy the improved configuration to production

When to use online testing:
- Quick validation of hypotheses with simulated user behavior
- A/B testing between configurations when you want engagement-based metrics
- Complementing offline evaluation (pointwise with judgments) with online-style testing
- When you want to see how configurations perform under realistic click patterns

Simulation best practices:
- Use at least 100 sessions for meaningful results (prefer 500+ for high confidence)
- position_based click model is most realistic for general search
- cascade model is good for testing if users find relevant results quickly
- Always check statistical significance (p-value) before declaring a winner
- Larger samples give more reliable results and higher confidence

Be concise, quantitative, and focus on statistical rigor. Always report confidence levels, p-values, and sample sizes.
Clearly distinguish between statistically significant and non-significant results.
"""
...
@monitored_tool(
    name="online_testing_agent",
    description="Runs interleaved A/B tests with simulated user behavior to compare search configurations. Creates tests, simulates realistic clicks and impressions, and determines winners with statistical significance.",
)
async def online_testing_agent(query: str) -> str:
    """
    Run interleaved A/B tests to compare search configurations using simulated user behavior.

    Args:
        query: A description of the online testing task (which configs to test, sample size, etc.)

    Returns:
        str: Test results with winner, click attribution, statistical significance, and insights
    """
    if not _opensearch_tools:
        return "Error: OpenSearch tools not configured. Please initialize MCP connection first."

    try:
        # Import interleaved testing tools
        ...
        # Create model with extended timeout configuration
        # Online testing with simulation can take 2-5 minutes depending on sample size
        boto_config = BotocoreConfig(
            connect_timeout=60,
            read_timeout=1200,  # 20 minutes for online testing with large simulations
            retries={"max_attempts": 0},  # Fail fast, no retries
        )
        model = BedrockModel(
            model_id=os.getenv("BEDROCK_INFERENCE_PROFILE_ARN"),
        ...
        # Combine utility tools with interleaved testing tools
        online_testing_tools = [
            # OpenSearch utility tools (direct API access)
            list_index,
            search_index,
            # Interleaved testing tools
            create_interleaved_test,
            get_interleaved_test,
            stop_interleaved_test,
            delete_interleaved_test,
            ...
            get_search_configuration,
            deploy_search_configuration_to_production,
            # Query set tools (read-only)
            get_query_set,
            # UBI analytics tools (to analyze generated events)
            get_query_ctr,
            get_document_ctr,
            get_query_performance_metrics,
            get_top_queries_by_engagement,
        ...
        # Create specialized agent with interleaved testing focus
        agent = Agent(
            model=model,
            system_prompt=ONLINE_TESTING_AGENT_SYSTEM_PROMPT,
            tools=online_testing_tools,
        )
        ...
        logger.exception("Error in online testing")
        error_msg = str(e)
        # Check for rate limit errors and return immediately without retry
        if "rate limit" in error_msg.lower() or "429" in error_msg:
            return "⚠️ Rate limit reached. Please wait a moment before trying again, or consider simplifying your request."
        return f"Error in online testing: {error_msg}"
